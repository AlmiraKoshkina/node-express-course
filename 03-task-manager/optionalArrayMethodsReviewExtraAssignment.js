...
const lastName = ["Smith", "Toure", "Hernandez"];
const initialValue = 0;
const totalLettersInNames = lastName.reduce((runningTotal, currentName) => {
...
const lastNames = names.map((fullName) => {
  const trimmed = fullName.trim();
  const parts = trimmed.split(/\s+/);
  return parts[parts.length - 1];
});

console.log("1) lastNames:", lastNames);

const myRightFormat = /^\w+ \w+$/;
const validNames = names.filter((fullName) => {
  return myRightFormat.test(fullName);
});

console.log("2) validNames:", validNames);

const toTitleCaseName = (fullName) => {
  return fullName
    .trim()
    .split(/\s+/)
    .map((word) => {
...
const titled = names.map(toTitleCaseName);
console.log("3) titled:", titled);

const myResult = names
  .filter((fullName) => myRightFormat.test(fullName)) // remove bad format
  .map(toTitleCaseName) // change to title case
  .filter((fullName) => {
    const lastLetter = fullName.slice(-1).toLowerCase();
    return lastLetter !== "z";
  })
  .map((fullName) => `Hey there ${fullName}! Want to sign up?`);

console.log("4) result:", myResult);
///////////////////////////////////////////////////////////////////////////////
